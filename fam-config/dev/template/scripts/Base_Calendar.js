...
/*
 * ! getWeekOfTheYear() was originally developed by Nick Baicoianu at
 * MeanFreePath: http://www.meanfreepath.com
 */
/**
...
Date.prototype.getWeekOfTheYear = function() {
  ...
  var day = newYear.getDay() - Base.Calendar.FIRST_DAY_OF_WEEK; // the day of
                                                                // week the year
                                                                // begins on
  day = (day >= 0 ? day : day + 7);
...
Base.Calendar.COLORS = [ "32C832", // available @see
                                    // FacilityAvailability#COMPLETE_AVAILABLE
"AAAAAA", // time beyond working hours or yesterdays @see
          // FacilityAvailability#GENERAL_NOT_AVAILABLE
"32C832", // partly available @see FacilityAvailability#MAYBE_AVAILABLE (colored
          // as available - partly color "FA9632")
...
"0000FF", // sudden failures @see
          // FacilityAvailability#SUDDEN_FAILURE_NOT_AVAILABLE
"C8FF32" // impossible to start a session here @see
...
Base.Calendar.timeAsText = function(minutes) {
  ...
  if (d > 0) {
    if (w > 0)
      result += (h > 0 || minutes > 0) ? ", " : " and "; // INTLANG
    result += d == 1 ? "1 day" : d + " days"; // INTLANG
  }
  if (h > 0) {
    if (w > 0 || d > 0)
      result += (minutes > 0) ? ", " : " and "; // INTLANG
...
Base.Calendar.initDateTimePicker = function() {
  Base.Calendar.dateTimePickerOptions.showHour = BookingRule.smallest_minutes_bookable % 1440 != 0;
  Base.Calendar.dateTimePickerOptions.showMinute = Base.Calendar.dateTimePickerOptions.showHour && BookingRule.smallest_minutes_bookable % 60 != 0;
  Base.Calendar.dateTimePickerOptions.hour = BookingRule.must_start_at ? Math.round(BookingRule.must_start_at / 60) : 0;
  Base.Calendar.dateTimePickerOptions.minute = 0;

};
Base.Calendar.dateTimePickerOptions = {};
      ...
      success : function(r) {
        if (r && r.succ) {
          ...
          if (!r.user_is_allowed_to_access) {
            ...
            // booking rule
            BookingRule = r.br;
            Base.Calendar.initDateTimePicker();

            // prepare date and time picker
            $('#j_start_date').datetimepicker(Base.Calendar.dateTimePickerOptions);
            $('#j_end_date').datetimepicker(Base.Calendar.dateTimePickerOptions);
            $('#j_start_date_fix_length').datetimepicker(Base.Calendar.dateTimePickerOptions);
            ...
            if (BookingRule.strategy == 1) { // time based
              ...
              $.n.error("please report error 201104121100-" + r.br + r.br.strategy); // =>
                                                                                      // unknown
                                                                                      // booking
                                                                                      // strategy
...
Base.JobSurvey.init = function(strat) {
  ...
  JobDataProcessing.getStructure(function(result) { // callback_succ
    ...
    if (result.has_job_data_processing) {
      ...
      if (result.templates && result.templates[0] && result.templates[0].behaviour) { // may
                                                                                      // have
                                                                                      // no
                                                                                      // template
        try {
...
Base.Calendar.Week.update = function(cEvents) {
  ...
  // ↘ go back from choosen date to the first day of week as defined by the
  // server(!).
  // ↘ assert that first element is first element of the week (@see java
  // heinzelmann TimeFrameFactory#getMonthWithFullWeeks())
...
/**
 * return an id for the given section. prepend the salt to the id. param salt
 * should be "month", "week" or "day" to prevent doubled ids on different
 * calendars.
 */
...
Base.Calendar.BarControl.showBarInElement = function(bar, element, dayWeekMonth) {
  ...
  $(bar.sections).each(function(i, section) {
    ...
    $("#" + secid).mouseup(function() {
      ...
      /*
       * save the mouse down element XXX does not work in all browsers
       */
...
/**
 * return the height of the given section if put in into given element. element
 * must be visible at this point, otherwise height will be 0.
 */
Base.Calendar.BarControl.getSectionHeight = function(section, element, dayWeekMonth) {
  // XXX assume, that height of month td is 120 and height of week td is 480 and
  // playing with nothing but hours
...
Base.Calendar.Dialog.BookingRequest.show = function(datetime_start, datetime_end) {
  if (datetime_start == null || datetime_end == null) {
    ...
    if(!Base.Calendar.dateTimePickerOptions.showHour) {
      var newHour = '';
      if(Base.Calendar.dateTimePickerOptions.hour < 10) newHour = '0';
      newHour += Base.Calendar.dateTimePickerOptions.hour;
      datetime_start = datetime_start.substr(0, 11) + newHour + ":00";
    }
      ...
      buttons : {
        'Check' : function(e) {
          ...
          if (Base.Calendar.Dialog.BookingRequest.isFixDate()) {
            ...
            var dateStart = new Date(data.request.start.substr(6, 4), data.request.start.substr(0, 2), data.request.start.substr(3, 2), data.request.start.substr(11, 2), data.request.start.substr(14, 2));
              ...
              try {
                if (r && r.succ) {
                  if (r.possibilities.length == 0) {
                    $('#js_booking_request_no_possibilities').show('slow').effect("highlight", {
                      color : 'red'
                    }, 3000);
                  } else {
                    var lis = '';
