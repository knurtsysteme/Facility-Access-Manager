...
public abstract class UserDao extends AbstractFamDao<User> {
  ...
  /**
   * return the user that have the given unique username. return null, if no user with given username exists.
   * 
   * @param username representing a user
  ...
  /**
   * return a unique username for given user username has 8 chars and is unique compared with all other users in the database. if the given user has a
   * username, this is the base to create the unique name. Otherwise the first and second name of the user is taken. If nothing is set at all, the
   * hashcode of the user object is taken. If the username is not unique, it gets a number: pemuelle, pemuell1, pemuell2, [...], pemuelle99.
   * 
   * #TODO Issue #29
   * 
   * @param user a unique username is created for
  ...
  /**
   * Return true, if another user <code>like</code> exists, that is not the same user as <code>andIsNotThis</code>.
   * 
   * @param like example user
   * @param andIsNotThis user that is the same user
   * @return true, if another user <code>like</code> exists, that is not the same user as <code>andIsNotThis</code>.
   */
  ...
  /**
   * Return true, if another user <code>like</code> given user exists. If the found user is the same user like the given, this returns false.
   * 
   * @param like user as example
  ...
  /**
   * store the given user if and only if it does not violate the data integrity. after validating the user succeeded, it delegates to
   * {@link #internInsert(de.knurt.fam.core.model.persist.Storeable)} always invoke {@link User#encodePassword}, so that only encoded passes are
   * saved.
   * 
   * @param user to store
   * @throws org.springframework.dao.DataIntegrityViolationException if storing would violate data integrity
   */
  @Override
  public synchronized boolean insert(User user) throws DataIntegrityViolationException {
    user.encodePassword();
    user.setHasBeenCreated();
   ...
   * @throws DataIntegrityViolationException if the user is not storeable (like it is when user has no name or mail address).
   */
  @Override
   ...
   * 
   * @param mail the given mail into the database.
   */
  public abstract boolean insert(UserMail mail);

  /**
   * return all users, that have an account and that are not barred from the system.
   * 
   * @return all users, that have an account and that are not barred from the system.
   */
  ...
  public synchronized boolean delete(List<ContactDetail> contactDetails) {
    ...
    boolean tmp = false;
    setChanged();
    notifyObservers(contactDetails);
  ...
  /**
   * set all personal user data to something anonym and update all relevated user data to this anonym user. delete all mails of the user and delete
   * all contactdetails.
   * 
   ...
   * </ul>
   * 
   * only change username of user's {@link LogbookEntry}s and {@link Booking}s to preserve logical information.
   * 
   * if the user <code>auth</code> does not have the right {@link FamAuth#ANONYMIZE_USER}, do nothing and return <code>false</code>.
   * 
   * @param user to anonymize
   * @param auth user calling this
  ...
  /**
   * return all users with the given role or an empty list if there is no user with the given role.
   * 
   * @param role the users have
