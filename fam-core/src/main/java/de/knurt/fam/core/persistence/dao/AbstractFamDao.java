...
/**
 * an abstract dao as base for all daos. provice the basic insert, update methods, but not a delete (which may not be wanted).
 * 
 * @param <K> the main storable object the dao is resolving. there may exist other storables resolved by the dao. but every dao has a main type of
 *          {@link Storeable} working with.<br />
 *          all other objects being resolved with this dao must depend from this main storable. e.g. the user dao also resolves mails stored for the
 *          user.
...
public abstract class AbstractFamDao<K extends Storeable> extends Observable implements FamDao<K> {
   ...
   * 
   * @param dataholder that shall be inserted or updated.
   * @param onInsert true, if dataholder shall be inserted. false, if it shall be updated.
   * @return true, if the state of dataholder violates the data integrity.
   */
   ...
   * 
   * @param dataholder the exception is thrown for
   * @throws DataIntegrityViolationException thrown because of given dataholder.
   */
   ...
   * 
   * @param dataholder the has been inserted.
   */
   ...
   * 
   * @param dataholder the has been updated.
   */
  protected abstract void logUpdate(K dataholder);

  /**
   * set the id for the given dataholder. this assumes that every dataholder needs an id before it is written into the database. this is a method to
   * do that. note: there may exist no need for that if it is done by the database rule (as it is on autoincrement values on relational databases).
   * 
   * @param dataholder the id is set to
   */
   ...
   * {@inheritDoc}
   * 
   * notify observers after successful insertion of the dataholder.
   */
  @Override
   ...
   * {@inheritDoc}
   * 
   * notify observers before the dataholder is updated, even if updating fails.
   */
  @Override
  ...
  /**
   * return first found object like example. or <code>null</code>, if no object like given stored
   * 
   * @param example as example
