...
import de.knurt.fam.core.model.persist.StoreableAbstract;
...
/**
 * a booking resolving things that are the same in all bookings of the access system.<br />
 * be sure, never have an User or Facility object directly as attribute here! it results in saving inconsistent data.
 * 
 * @author Daniel Oltmanns
 * @since 0.20090827
 */
public abstract class AbstractBooking extends StoreableAbstract implements Booking {
  ...
  /**
   * construct a booking for a user, a facility, with a status and capacity units and booked by rules.
   * 
   * @param facilityKey representing the facility this booking is for
   * @param username of user booked this
   * @param bookingStatus the status of booking ({@link BookingStatus})
   * @param capacityUnits units booked
   * @param bookingRule rules used for this booking
   */
  ...
  /**
   * send the applicant, that requested this booking, a confirmation and give him the bonus for it.
   * 
   * @param mailMessage additional message to this confirmation
   */
  @Override
  ...
  /**
   * return -1, if this booking starts before the other booking. otherwise return 1.
   * 
   * @param otherBooking other booking
   * @return -1, if this booking starts before the other booking. otherwise return 1.
   */
  ...
  /**
   * return true, if this booking is an application. same as <code>this.getBookingStatus().isApplied()</code>.
   * 
  ...
  /**
   * return true, if the booking is complete available. do the same as {@link #isAvailableForInsertion()}
   * 
