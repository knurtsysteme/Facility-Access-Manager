...
public class LogbookMakePostModelFactory {
	...
	public Properties getProperties(TemplateResource templateResource) {
		...
		String logbookId = this.getLogbookKey(templateResource.getRequest());
		if(logbookId == null) {
			logbookId = LogbookConfigDao.getInstance().getAll().get(0).getKey();
		}
		result.put("logbook_label", LogbookConfigDao.getInstance().getLabel(logbookId));
		result.put("logbook_description", LogbookConfigDao.getInstance().getDescription(logbookId));
		result.put("logbook_query", QueryStringBuilder.getLogbookQueryString(logbookId));
		result.put("logbook_musttags", LogbookConfigDao.getInstance().getTags(logbookId));
		result.put("logbook_logbookId", logbookId);
